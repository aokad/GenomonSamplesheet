...
$Id: create_controlpanel.py 202 2017-07-10 09:48:38Z aokada $
$Rev: 202 $
"""
...
# 省略可能でデフォルト時の設定は以下
# default = {
#     "large_controlpanel": False,    # 大きな一つのコントロールサンプルを作る
#     "set_controlpanel_length": 20,  # コントロールパネル１リストの長さ
# }
...
# }
#
def create_controlpanel(input_samples, input_controls, options = {}):
    ...
    PANEL_LENGTH = options["set_controlpanel_length"]
    
    
    if control_num < PANEL_LENGTH:
        print ("ERROR!!! Too few control number.[%d]" % control_num)
        return return_obj
    
    if options["large_controlpanel"]:
        return_obj = controlpanel_method3(input_samples, input_controls)
    else:
        # controlの数で処理を割りふる    
        list_num = 0
        shift_range = 0
        ...
        elif control_num < PANEL_LENGTH*2:
            list_num = 2
            shift_range = control_num-PANEL_LENGTH
    
        else: # control_num >= 40
            capacity_list = split_list(control_num, persons = PANEL_LENGTH)
            list_num = len(capacity_list)
            shift_range = PANEL_LENGTH
        ...
        return_obj = controlpanel_method2(mix_samples, mix_controls, max_member, shift_range, PANEL_LENGTH)
    
    if return_obj != None:
        print ("created controlpanel. %d persons, %d controlpanel lists" % (len(input_samples), len(return_obj["controlpanel"])))
...
#
# 一つの巨大なコントロールパネルを作る
def controlpanel_method3(input_samples, input_controls):

    new_obj = {"index": 0, "samples": [], "persons": [], "uses": len(input_samples)}
    ...
    new_obj["samples"].extend(sorted(list(set(samples))))
    new_obj["persons"].extend(sorted(list(set(persons))))
    
    if len(new_obj["samples"]) < 20:
        print ("ERROR!!! Too few control number.[%d]" % len(new_obj["samples"]))
        return None
        
    tumor_panel = {}
    
    for tumor in input_samples:
        tumor_panel[tumor["sample"]] = 0
                
    return tools_format_obj(tumor_panel, [new_obj])
...
def controlpanel_method2(input_samples, input_controls, max_member, shift_range, PANEL_LENGTH):
    ...
    for tumor in input_samples:
        ...
        if along != None:
            ...
            for retry in range(3):
                # 自分用のリストを作る
                new_obj = tools_selftlist(tumor, input_controls, shift, PANEL_LENGTH)
...
def controlpanel_method1(input_samples, input_controls, PANEL_LENGTH):
...
#
# 自分を含まないコントールパネルを作る
def tools_selftlist(tumor, input_controls, shift, PANEL_LENGTH):
    ...
    for c in range(len(input_controls)):
        ...
        if len(new_obj["samples"]) >= PANEL_LENGTH:
            break
